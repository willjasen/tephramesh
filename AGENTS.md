...
- The Obsidian File Explorer prefixes Markdown notes that any active mesh instance still needs with a prominent blue Obsidian `refresh-cw` icon. This is a DOM decoration only and never changes filenames or note contents. Tephramesh polls Syncthing local- and remote-need data at the configured note sync icon interval, preferring the primary device and falling back to other reachable devices; it retains the last known indicators if no active device can answer. Shards are checked as remote peers but are not query sources because their local filenames are encrypted.
...
- Required validation for code changes: run `npm test` and `npm run build`. The current baseline is 40 tests across eight test files; update this count here only when it materially helps orientation.
