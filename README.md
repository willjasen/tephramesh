...
i've seen a lot of interest in syncing obsidian vaults. naturally, there is obsidian sync but it's a paid service (though the team should be supported - i have a catalyst license). i've tried icloud and it works well until it doesn't. syncthing is distributed and private, so why not - let's make it happen.

this project's idea stems from the idea of private distributred storage, an idea i've explored before with [rainstash](https://github.com/willjasen/rainstash).
