...
next is to add another host. there are two types of hosts within tephramesh - devices and shards.

- devices are generally a desktop, laptop, phone, etc. where obsidian, syncthing, and tephramesh will run - devies have send/receive permissions for the vault
- shards are generally online and are hosts that will store an encrypted copy of the syncthing folder - shards do not interact with an obsidian vault directly

tephramesh handles sharing and accepting the vault folder between syncthing devices.
...
this project's idea stems from the idea of private distributred storage and darkclouds, an idea i've explored before with [rainstash](https://github.com/willjasen/rainstash).

the rest of this README was generated by codex:
...
- A shard's peer-trust setting is left unchanged; its managed folder type is `receiveencrypted`.
...
- Tephramesh uses two schema-version boundaries: the plaintext envelope currently uses `schemaVersion: 3`, and the age-decrypted payload currently uses `schemaVersion: 1`. The decrypted payload contains `settings` and `secrets`; neither nested object has a separate schema version. Older encrypted snapshots with redundant nested schema fields remain readable and are normalized when unlocked.
...
- The topology preview validates full-mesh folder modes, managed device-to-device trust, and encryption-password placement, while preserving peer trust for any relationship involving a shard; it also shows the highest live global-file count reported by the mesh.
